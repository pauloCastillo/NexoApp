...
class BranchAssignmentDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BranchAssignmentDialog> createState() => _BranchAssignmentDialogState();
}

class _BranchAssignmentDialogState extends ConsumerState<BranchAssignmentDialog> {
    ...
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: branchesAsync.when(
          data: (branches) {
            if (branches.isEmpty) return const Text('No hay sucursales creadas. Crea una en Empresa → Sucursales.');
            return Wrap(spacing: 8, runSpacing: 8, children: branches.map((b) {
              final sel = _selected.contains(b.id);
              return FilterChip(
                label: Text('${b.name} (${b.geofenceRadius}m)'),
                selected: sel,
                onSelected: (v) => setState(() { if (v) _selected.add(b.id); else _selected.remove(b.id); }),
                avatar: Icon(Icons.store, size: 16, color: sel ? Theme.of(context).colorScheme.primary : null),
              );
            }).toList());
          },
  ...
  Future<void> _save() async {
    if (_selected.length > 10) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Máximo 10 zonas por empleado')));
      return;
    }
    setState(() => _saving = true);
    try {
      final src = ref.read(branchRemoteSourceProvider);
      final res = await src.assignBranchesResult(widget.employee.id, _selected.toList());
